package de.artur.coinapi.api;

import de.artur.coinapi.Main;
import de.artur.coinapi.MySQL;
...
public class CoinAPI{
    ...
    public static Integer getCoins(UUID uuid) {
        ...
        try {
            ResultSet rs = Main.mysql.query("SELECT * FROM coinapi WHERE UUID= '" + uuid + "'");
    ...
    public static void setCoins(UUID uuid, int amount) {
        if (playerExists(String.valueOf(uuid))) {
            Main.mysql.update("UPDATE coinapi SET COINS= '" + amount + "' WHERE UUID= '" + uuid + "';");
        } else {
            createPlayer(String.valueOf(uuid));
    ...
    public static void createPlayer(String uuid) {
        if (!playerExists(uuid)) {
            Main.mysql.update("INSERT INTO coinapi(UUID, COINS) VALUES ('" + uuid + "', '1000');");
        }
